...
export default function ContactPage() {
  ...
  const { value: info } = useSiteConfig<{
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 space-y-3 rounded-lg border border-coffee-950/10 bg-white p-5">
          ...
          {info.email && (
            <a
              ...
              className="flex items-center gap-3 text-sm text-ink-800 hover:text-clay-600"
            >
              <Mail
                size={18}
      ...
      {/* Success Message */}
      {status === 'sent' ? (
        <p className="mt-8 rounded-md bg-canopy-700/10 px-4 py-3 text-sm text-canopy-700">
          {t.success}
        </p>
      ) : (
        /* Contact Form */
